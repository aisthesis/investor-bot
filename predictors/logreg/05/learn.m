...
ifTrain = sprintf("%s/splitadj/closes/train60xval20test20/train/features/%d/labels/%d/%s/%dpct/mean0/combined.mat", ...
    PREDICTOR_DATA_ROOT, featureInterval, labelInterval, labelType, floor(ratio * 100));
ifXval = sprintf("%s/splitadj/closes/train60xval20test20/xval/features/%d/labels/%d/%s/%dpct/mean0/combined.mat", ...
    PREDICTOR_DATA_ROOT, featureInterval, labelInterval, labelType, floor(ratio * 100));
