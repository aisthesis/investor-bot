...
## Save predictions for target equities in csv format.
##
## @itemize
## @item
## The file will include a header row with the titles
## @code{Date} and @code{Label}
##
## @item
...
## @itemize
## @item
## @code{getPredictions()} must be run prior to running this script.
## @end itemize
##
## Usage:
## 
...
function csvOutput()
    ...
    for i = 1:nEq
        ...
        printfNow(". ");
        for j = 1:nrows
            fprintf(fid, "\n%s,%d", datestr(currDate(j), "yyyy-mm-dd"), label(j));
            if mod(j, 64) == 63
