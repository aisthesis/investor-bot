...
def get_alldata():
    ...
    n_pred_intervals = 7
    fname = "../{0}/features/growth/relvol/sma20/{1}/labels/multi/{2}/combined.npy".format(settings.DATA_ROOT,
...
def _get_featfuncs(pricecol):
    ...
    # Relative volume
    _funcs.append(pn.decorate(partial(pn.tech.ratio_to_ave, averaging_interval=_averaging_interval),
            title='RelVol'))            
    ...
    _funcs.append(pn.decorate(partial(pn.tech.growth, selection=pricecol, n_sessions=_sma_window), 
            title='SMAGrowth'))
