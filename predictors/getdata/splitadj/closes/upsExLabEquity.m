function [y] = upsExLabEquity(ticker, interval, minRatio)
...
displayNow("upsExLabEquity() called");
INFILE_ROOT = getenv("DATA_ROOT");
INFILE_SUFFIX = "-splitadj.mat";
OUTFILE_ROOT = getenv("PREDICTOR_DATA_ROOT");
path = {"labels" sprintf("%d", interval) "upside-exceeded" sprintf("%dpct", floor((minRatio - 1) * 100))}; 
...
load(inFile);
y = getUpsideExceededLabels(closes, interval, minRatio);
